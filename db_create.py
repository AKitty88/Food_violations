...
cursor.execute(sql)                                                 # score number(3) !!! TEST
...
for row in sheet_ins.iter_rows(min_row=2, max_row=5):               # delete max_row=5 !!! TEST
    ...
    #cursor.execute("INSERT INTO inspection VALUES('{acd}', '{eid}', '{fad}', '{fac}', '{fai}', '{fan}', '{fas}', '{faz}', '{g}', '{oi}', '{on}', '{pd}', {pep}, '{pn}', '{ps}', '{ri}', '{s}', '{sn}', {sc}, '{sd}');".format(acd=activity_date, eid=employee_id, fad=facility_address, fac=facility_city, fai=facility_id, fan=facility_name, fas=facility_state, faz=facility_zip, g=grade, oi=owner_id, on=owner_name, pd=pe_description, pep=program_element_pe, pn=program_name,  ps=program_status, ri=record_id, s="0", sn=serial_number, sc=service_code, sd=service_description))
