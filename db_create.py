...
for row in sheet_ins.iter_rows(min_row=2, max_row=5):
    ...
    #print("INSERT INTO inspection VALUES('"+str(activity_date)+"', '"+employee_id+"', facility_address, facility_city, facility_id, facility_name, facility_state, facility_zip, grade, owner_id, owner_name, pe_description, program_element_pe, program_name, program_status, record_id, score, serial_number, service_code, service_description);")
    #cursor.execute("INSERT INTO inspection (activity_date, employee_id) VALUES('{acd}', '{eid}');".format(acd=activity_date, eid=employee_id))
    cursor.execute("INSERT INTO inspection (activity_date, employee_id, facility_address, facility_city, facility_id, facility_name, facility_state, facility_zip, grade, owner_id, owner_name, pe_description, program_element_pe, \
                                            program_name, program_status, record_id, score, serial_number, service_code, service_description) VALUES('{acd}', '{eid}', '{fad}', '{fac}', '{fai}', '{fan}', '{fas}', '{faz}', '{g}', \
                                            '{oi}', '{on}', '{pd}', '{pep}', '{pn}', '{ps}', '{ri}', '{s}', '{sn}', '{sc}', '{sd}');".format(acd=activity_date, eid=employee_id, fad=facility_address, fac=facility_city, \
                                            fai=facility_id, fan=facility_name, fas=facility_state, faz=facility_zip, g=grade, oi=owner_id, on=owner_name, pd=pe_description, pep=program_element_pe, pn=program_name, \
                                            ps=program_status, ri=record_id, s=score, sn=serial_number, sc=service_code, sd=service_description))
